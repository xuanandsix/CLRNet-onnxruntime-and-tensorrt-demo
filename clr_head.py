...
class CLRHead(nn.Module):
    def __init__(self,
        ...
        self.sample_x_indexs = (torch.linspace(0, 1, steps=self.sample_points, dtype=torch.float32) * self.n_strips).long()
        self.prior_feat_ys = torch.flip((1 - self.sample_x_indexs.float() / self.n_strips), dims=[-1])
        self.prior_ys = torch.linspace(1, 0, steps=self.n_offsets, dtype=torch.float32)
